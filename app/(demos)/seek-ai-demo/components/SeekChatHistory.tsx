...
const ChatHistory: React.FC<ChatHistoryProps> = ({ chatTitle, messages, setChatHistory, chatEndRef }) => {
  ...
  return (
    <>
      <div className="w-full pb-4 rounded-lg mb-4 bg-transparent text-center">
        {/* Chat Title */}
        <div className="text-center pt-3 mb-3 sticky top-0 ">
          <div className="inline-flex flex-row items-center justify-center border rounded-2xl text-sm font-semibold mx-auto py-2 px-4">
            {!chatTitle ? `New Chat ${formattedCurrentDate()}` : chatTitle}
            ...
            {messages.map((msg, index) => (
              <li
                ...
                className={`flex flex-col items-start gap-1 ${msg.type === "user"
                  ? "items-end px-4 py-2 rounded-2xl bg-blue-800 dark:bg-gray-700 text-white dark:text-white self-end ml-auto text-right w-fit max-w-[75%]"
